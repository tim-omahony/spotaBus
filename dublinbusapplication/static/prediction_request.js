...
function submitForm() {
    ...
    $.ajax({
            ...
            success: function (result) {
                ...
                //checking if the users inputted time is greater than five days from now, only displaying weather
                //forecast if it isn't as our forecast from openweathermap only accounts for five days ahead
                const unix_now = new Date().getTime();
                const unix_five_days = unix_now + (5 * 86400*1000)
                if (inputTime.getTime() > unix_five_days) {
                    document.getElementById('parenttwo').style.display = 'none';
                } else {
                    document.getElementById('parenttwo').style.display = 'flex';
                }
            },
